...
EFI_SMM_IO_TRAP_REGISTER_CONTEXT m_IoTrapContext = {0,0,WriteTrap };

//--------------------------------------------------------------------------
EFI_SMM_PERIODIC_TIMER_DISPATCH2_PROTOCOL* m_PeriodicTimerDispatch = NULL;

EFI_SMM_IO_TRAP_DISPATCH2_PROTOCOL* m_IoTrapDispatch = NULL;
//---------------------------------------------------------------------timer
...
EFI_STATUS EFIAPI IoTrapDispatch2Handler(EFI_HANDLE DispatchHandle, CONST VOID* Context, VOID* CommBuffer, UINTN* CommBufferSize)
{
	TurnOnSpeaker();
	return EFI_SUCCESS;
}
...
EFI_STATUS IoTrapDispatch2Register(EFI_HANDLE* DispatchHandle)
{//레지스터
	EFI_STATUS Status = EFI_INVALID_PARAMETER;
	...
	{
		// register periodic timer routine
		Status = m_IoTrapDispatch->Register(
			m_IoTrapDispatch,
			IoTrapDispatch2Handler,
			&m_IoTrapContext,
			DispatchHandle
		);
		if (Status == EFI_SUCCESS)
...
EFI_STATUS EFIAPI IoTrapDispatch2ProtocolNotifyHandler( //있으면 이걸 바로 부르는 거구나.. 바로 불려오면...
	CONST EFI_GUID* Protocol,
	VOID* Interface,
	EFI_HANDLE Handle)
{
	...
	m_IoTrapDispatch = (EFI_SMM_IO_TRAP_DISPATCH2_PROTOCOL*)Interface;
...
VOID HmonEntrySmm(VOID)
{
	...
	EFI_SMM_IO_TRAP_DISPATCH2_PROTOCOL* IoTrapDispatch = NULL;
...
#define REGISTER_NOTIFY(_name_) RegisterProtocolNotifySmm(&gEfiSmm##_name_##ProtocolGuid,   _name_##ProtocolNotifyHandler, &Registration)
	...
	Status = gSmst->SmmLocateProtocol(&gEfiSmmIoTrapDispatch2ProtocolGuid, NULL, &IoTrapDispatch);
	if (Status == EFI_SUCCESS)//있으면 핸들러를 바로 부르고..
	{
		// protocol is already present, call handler directly
		IoTrapDispatch2ProtocolNotifyHandler(&gEfiSmmIoTrapDispatch2ProtocolGuid, IoTrapDispatch, NULL);
	}
	else
	{
		REGISTER_NOTIFY(IoTrapDispatch2);
	}
